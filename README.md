...
不同于 Capslock++ 等软件，本程序实现了真正的按键替换。比如你输入法选字的时候也可以用，代码提示选代码也可以用。还可以让 大小写键和 CTRL 等按键集成，比如一般状态下 Ctrl+← 是向左跳一个单词，使用本工具可以让 Ctrl+大小写+H 同样实现向左跳一个单词。而且本工具体积更小，只有 50kb 上下。
